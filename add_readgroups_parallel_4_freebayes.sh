...
# note the path to picard, this should be changed if using a different machine.
# /data/programs/picard-tools-1.139/picard.jar
...
### Run picard tools AddreplaceRGs in serial

    # java -Xmx10g  -jar /data/programs/picard-tools-1.139/picard.jar AddOrReplaceReadGroups I=$bam_path/${sampleID} O=$bam_path/${sampleID%.bam}.rg.bam RGID=${flow_cell}.${lane}.${simpleID} RGLB=${simpleID}.${seqnum} RGPL=${instrument} RGSM=${simpleID} RGPU=${flow_cell}.${lane}.${barcode}

# to run in parallel mode, with one core per bam file (only do < 40)
    nohup java -Xmx10g  -jar /data/programs/picard-tools-1.139/picard.jar AddOrReplaceReadGroups I=$bam_path/${sampleID} O=$bam_path/${sampleID%.bam}.rg.bam RGID=${flow_cell}.${lane}.${simpleID} RGLB=${simpleID}.${seqnum} RGPL=${instrument} RGSM=${simpleID} RGPU=${flow_cell}.${lane}.${barcode} &>/dev/null &
